...
predates the v0.49.14 dispatch contract, and the rerun it requires has not been performed:

- v0.49.14 live smoke evidence: untested

`untested` is a declaration, not evidence. It means no check in this document was executed against
the v0.49.14 contract, so the opencode host wiring is statically verified only. Contract
...
`- v0.49.14 live smoke evidence: date=YYYY-MM-DD; opencode=X.Y.Z; checks=1-7; sha256=<64 lowercase hex>; location=evidence/opencode-v0.49.14/manifest.json`
...
node tools/agentic/release-verify.mjs \
  --fingerprint-stdin <evidence/opencode-v0.49.14/manifest.json
```
