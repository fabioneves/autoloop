...
export function runSmokeSteps({
  ...
  timed('posture-audit', () => {
    ...
    // The invariant is that a reviewer can never write: permission mode plan
    // and a tool ceiling without Write, Edit or Bash.
    const reviewerLaunches = launched.filter((line) =>
      line.includes('--permission-mode plan'));
    const offending = reviewerLaunches.filter((line) => /--tools \S*(?:Write|Edit|Bash)/.test(line));
    ...
    return {
      ok: true,
      detail: `${reviewerLaunches.length} reviewer launch(es), none with a write capability`,
    };
  });
