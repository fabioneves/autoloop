...
export const LOOP_BRANCH_RE = /^(?:feat|fix|chore|docs|refactor|test|perf|build|ci)\/gh-(?<issue>[1-9]\d*)-/;
...
export function parseLoopClaim(input = {}) {
  ...
  const branchMatch = LOOP_BRANCH_RE.exec(branch);
  const bodyMatches = [...body.matchAll(new RegExp(CLOSES_RE.source, 'gi'))];
  const branchIssue = issueNumber(branchMatch?.groups?.issue);
