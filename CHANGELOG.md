...
- **A unit's cost on its issue.** Each dispatch logs its `--issue` and branch, and the run record
  ends with `stats.mjs --record`: active time per step summed across sessions (paused while
  blocked or waiting), with the dispatches behind each step (role, model, effort, duration,
  failure, fallback) and an `autoloop-timing-v1` marker.
